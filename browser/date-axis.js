...
(function (root) {
  ...
  const JDN_MINUS_PROJECT_DAY = 1721425n;

  function floorDiv(aValue, bValue) {
  ...
  function isGregorianLeapYear(yearValue) {
  ...
  function validateGregorian(date, fieldName) {
    const field = fieldName || 'Li date';
    if (!date || typeof date !== 'object') throw new TypeError(field + ' ne es un valid date.');
    if (!Number.isInteger(date.month) || date.month < 1 || date.month > 12) {
      throw new RangeError(field + ': li mensu deve esser inter 1 e 12.');
    }
    const maxDay = daysInGregorianMonth(date.year, date.month);
    if (!Number.isInteger(date.day) || date.day < 1 || date.day > maxDay) {
      throw new RangeError(field + ': li die ne existe in li selectet mensu.');
    }
    return date;
  }
  ...
  function parseIsoDate(value, fieldName) {
    ...
    if (value instanceof Date) {
      if (Number.isNaN(value.getTime())) throw new RangeError(field + ' ne es un valid date.');
      return Object.freeze({
    ...
    const match = /^([+-]?\d+)-(\d{2})-(\d{2})$/.exec(String(value == null ? '' : value).trim());
    if (!match) throw new RangeError(field + ' deve usar li formate YYYY-MM-DD.');
    return Object.freeze(validateGregorian({
      year: BigInt(match[1]),
  ...
  function gregorianToJdn(dateValue) {
    const date = validateGregorian(dateValue, 'Li date');
  ...
  function projectDayToJdn(dayValue) {
    return BigInt(dayValue) + JDN_MINUS_PROJECT_DAY;
  }

  function jdnToProjectDay(jdnValue) {
  ...
  ns.dateAxis = Object.freeze({
    JDN_MINUS_PROJECT_DAY,
    floorDiv,
    isGregorianLeapYear,
