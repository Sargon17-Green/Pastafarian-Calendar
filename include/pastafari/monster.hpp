...
struct LegacyYearMembershipReport {
    ...
    Patch18YearRecord legacyOutputYearBeforePatch{};
    Patch18YearRecord correctOutputYear{};
    std::size_t legacyForwardStepsBeforePatch = 0;
    std::size_t legacyBackwardStepsBeforePatch = 0;
    std::size_t correctForwardSteps = 0;
    std::size_t correctBackwardSteps = 0;
    bool patch26LegacyExecuted = false;
    bool patch26CorrectComputed = false;
    bool patch26LegacyEqualsCorrect = false;
    bool patch26LegacyReturned = false;
    bool patch26AuthoritativeIntervalAccepted = false;
    bool patch26Applied = false;
};

struct Patch26YearMembershipDecision {
    Patch18YearRecord legacyOutputYear{};
    Patch18YearRecord correctOutputYear{};
    Patch18YearRecord outputYear{};
    std::size_t correctForwardSteps = 0;
    std::size_t correctBackwardSteps = 0;
...
struct MonthWeavingPatchDecision {
    Integer legalFamilyCount{};
    Integer wantedRank{};
    std::vector<int> correctWeaving{};
    std::vector<int> outputWeaving{};
    bool legacyExecuted = false;
    bool correctComputed = false;
    bool ghostEqualsCorrect = false;
    bool legacyReturned = false;
    bool semanticWholeWeavingOrderLegal = false;
    bool patchApplied = false;
};
...
struct LegacyMonthWeavingReport {
    ...
    bool wholeWeavingOrderLegal = false;
    bool legacyUsedAsSemanticOutput = false;
    Integer patch24LegalFamilyCount{};
    Integer patch24WantedRank{};
    std::vector<int> patch24CorrectWeaving{};
    bool patch24LegacyExecuted = false;
    bool patch24CorrectComputed = false;
    bool patch24GhostEqualsCorrect = false;
    bool patch24LegacyReturned = false;
    bool patch24SemanticWholeWeavingOrderLegal = false;
    bool patch24Applied = false;
    bool ready = false;
    std::string phase;
    std::string status;
    std::string handler;
...
struct LegacyContiguousMonthDayReport {
    Integer calculationDay{};
    Integer originalTargetDay{};
    Patch18YearRecord resolvedYear{};
    std::vector<int> monthLengths{};
...
struct BaseMonsterContext {
    ...
    bool patch24SemanticWholeWeavingOrderLegal=false;
    bool patch24Applied=false;
    bool patch24MonthWeavingReady=false;
    std::size_t discovery25TargetPosition1=0;
    int discovery25TargetMonthId=0;
    std::size_t discovery25FirstOccurrencePosition1=0;
    int discovery25LegacyGuessedDayInMonth=0;
    int discovery25SemanticDayInMonth=0;
    bool discovery25Patch24Prepared=false;
    ...
    Patch18YearRecord patch26CorrectOutputYear{};
    std::size_t patch26CorrectForwardSteps=0;
    std::size_t patch26CorrectBackwardSteps=0;
    bool patch26LegacyExecuted=false;
    bool patch26CorrectComputed=false;
    bool patch26LegacyEqualsCorrect=false;
    bool patch26LegacyReturned=false;
...
public:
    ...
    void requireDiscovery24MonthWeavingReady(
        const BaseMonsterContext& ctx) const;
    void requirePatch24MonthWeavingReady(
        const BaseMonsterContext& ctx) const;
...
class OpeningGateMembershipPatchWrapper {
public:
    Patch26YearMembershipDecision repair(
        const Integer& calculationDay,
        const LegacyYearAnchor& anchor,
        const Integer& targetDay,
...
public:
    MonthWeavingPatchDecision repair(
        const std::vector<int>& lengths,
...
class Patch24MonthWeavingHandler {
public:
    void handle(BaseMonsterContext& ctx,
...
class Patch26OpeningGateYearMembershipHandler {
public:
    void handle(BaseMonsterContext& ctx,
...
public:
    ...
    void dispatchDiscovery24MonthWeaving(
        ...
        const BaseValidationManager& validator,
        const BaseMetricsShell& metrics) const;
    void dispatchPatchedMonthWeaving(
        BaseMonsterContext& ctx,
        ...
        const BaseValidationManager& validator,
        const BaseMetricsShell& metrics) const;
    void dispatchDiscovery25ContiguousMonthDay(
        BaseMonsterContext& ctx,
    ...
    void dispatchDiscovery26OpeningGateYearMembership(
        ...
        const BaseValidationManager& validator,
        const BaseMetricsShell& metrics) const;
    void dispatchPatchedOpeningGateYearMembership(
        BaseMonsterContext& ctx,
...
public:
    ...
    LegacyMonthWeavingReport executeUnpatchedDiscovery24MonthWeavingDiagnostic(
        const LegacyYearAnchor& anchor,
        const Integer& originalTargetDay,
        const Integer& calculationDay,
        const std::vector<int>& monthLengths) const;
    ...
    LegacyYearMembershipReport executeDiscovery26OpeningGateYearMembership(
        ...
        const Integer& targetDay,
        const Integer& calculationDay) const;
    LegacyYearMembershipReport executeUnpatchedDiscovery26OpeningGateYearMembershipDiagnostic(
        const LegacyYearAnchor& anchor,
        const Integer& targetDay,
        const Integer& calculationDay) const;
