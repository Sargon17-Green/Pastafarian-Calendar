(module
  ...
  ;; Residuos 0..M-1 en base 2^30. Esta capa solo se usa donde la referencia aplica SAVE.
  (global $R_M i32 (i32.const 4096))
  (global $R_TMP0 i32 (i32.const 4144))
  (global $R_TMP1 i32 (i32.const 4192))
  (global $R_ACTION i32 (i32.const 4240))
  (global $R_TARGET i32 (i32.const 4288))
  (global $R_DISTANCE i32 (i32.const 4336))
  (global $R_CONNECTION i32 (i32.const 4384))
  (global $R_DIRECTION i32 (i32.const 4432))
  (global $R_CELL i32 (i32.const 48))

  ;; Formato de residuo: len i32 seguido de hasta 6 miembros i32.
  (func $r_len (param $p i32) (result i32) (i32.load (local.get $p)))
  (func $r_set_len (param $p i32) (param $n i32) (i32.store (local.get $p) (local.get $n)))
  (func $r_addr (param $p i32) (param $i i32) (result i32)
    (i32.add (i32.add (local.get $p) (i32.const 4)) (i32.mul (local.get $i) (i32.const 4))))
  (func $r_limb (param $p i32) (param $i i32) (result i32) (i32.load (call $r_addr (local.get $p) (local.get $i))))
  (func $r_set_limb (param $p i32) (param $i i32) (param $v i32)
    (if (i32.ge_u (local.get $i) (i32.const 6)) (then unreachable))
    (i32.store (call $r_addr (local.get $p) (local.get $i)) (local.get $v)))
  (func $r_zero (param $p i32) (call $r_set_len (local.get $p) (i32.const 0)))
  (func $r_normalize (param $p i32)
    (local $n i32)
    (local.set $n (call $r_len (local.get $p)))
    (block $done (loop $again
      (br_if $done (i32.eqz (local.get $n)))
      (br_if $done (i32.ne (call $r_limb (local.get $p) (i32.sub (local.get $n) (i32.const 1))) (i32.const 0)))
      (local.set $n (i32.sub (local.get $n) (i32.const 1)))
      (br $again)))
    (call $r_set_len (local.get $p) (local.get $n)))
  (func $r_copy (param $dst i32) (param $src i32)
    (local $i i32) (local $n i32)
    (local.set $n (call $r_len (local.get $src)))
    (call $r_set_len (local.get $dst) (local.get $n))
    (local.set $i (i32.const 0))
    (block $done (loop $again
      (br_if $done (i32.ge_u (local.get $i) (local.get $n)))
      (call $r_set_limb (local.get $dst) (local.get $i) (call $r_limb (local.get $src) (local.get $i)))
      (local.set $i (i32.add (local.get $i) (i32.const 1)))
      (br $again))))
  (func $r_from_u32 (param $p i32) (param $x i32)
    (if (i32.eqz (local.get $x))
      (then (call $r_zero (local.get $p)))
      (else (call $r_set_len (local.get $p) (i32.const 1)) (call $r_set_limb (local.get $p) (i32.const 0) (local.get $x)))))
  (func $r_init_m
    (call $r_set_len (global.get $R_M) (i32.const 5))
    (call $r_set_limb (global.get $R_M) (i32.const 0) (i32.const 1073741823))
    (call $r_set_limb (global.get $R_M) (i32.const 1) (i32.const 1073741823))
    (call $r_set_limb (global.get $R_M) (i32.const 2) (i32.const 1073741823))
    (call $r_set_limb (global.get $R_M) (i32.const 3) (i32.const 1073741823))
    (call $r_set_limb (global.get $R_M) (i32.const 4) (i32.const 127)))
  (func $r_cmp (param $a i32) (param $b i32) (result i32)
    (local $na i32) (local $nb i32) (local $i i32) (local $av i32) (local $bv i32)
    (local.set $na (call $r_len (local.get $a))) (local.set $nb (call $r_len (local.get $b)))
    ...
    (block $eq (loop $again
      ...
      (local.set $i (i32.sub (local.get $i) (i32.const 1)))
      (local.set $av (call $r_limb (local.get $a) (local.get $i)))
      (local.set $bv (call $r_limb (local.get $b) (local.get $i)))
      (if (i32.lt_u (local.get $av) (local.get $bv)) (then (return (i32.const -1))))
      (if (i32.gt_u (local.get $av) (local.get $bv)) (then (return (i32.const 1))))
      (br $again)))
    (i32.const 0))
  (func $r_sub (param $dst i32) (param $a i32) (param $b i32)
    (local $na i32) (local $nb i32) (local $i i32)
    (local $borrow i64) (local $av i64) (local $bv i64) (local $v i64)
    (if (i32.lt_s (call $r_cmp (local.get $a) (local.get $b)) (i32.const 0)) (then unreachable))
    (if (i32.eq (local.get $dst) (local.get $a)) (then (call $r_copy (global.get $R_TMP1) (local.get $a)) (local.set $a (global.get $R_TMP1))))
    (local.set $na (call $r_len (local.get $a))) (local.set $nb (call $r_len (local.get $b)))
    (local.set $i (i32.const 0)) (local.set $borrow (i64.const 0))
    (block $done (loop $again
      (br_if $done (i32.ge_u (local.get $i) (local.get $na)))
      (local.set $av (i64.extend_i32_u (call $r_limb (local.get $a) (local.get $i))))
      (local.set $bv (if (result i64) (i32.lt_u (local.get $i) (local.get $nb)) (then (i64.extend_i32_u (call $r_limb (local.get $b) (local.get $i)))) (else (i64.const 0))))
      (local.set $v (i64.sub (i64.sub (local.get $av) (local.get $bv)) (local.get $borrow)))
      (if (i64.lt_s (local.get $v) (i64.const 0))
        (then (local.set $v (i64.add (local.get $v) (global.get $BASE))) (local.set $borrow (i64.const 1)))
        (else (local.set $borrow (i64.const 0))))
      (call $r_set_limb (local.get $dst) (local.get $i) (i32.wrap_i64 (local.get $v)))
      (local.set $i (i32.add (local.get $i) (i32.const 1))) (br $again)))
    (if (i64.ne (local.get $borrow) (i64.const 0)) (then unreachable))
    (call $r_set_len (local.get $dst) (local.get $na)) (call $r_normalize (local.get $dst)))
  (func $r_double_bit (param $p i32) (param $bit i32)
    (local $i i32) (local $n i32) (local $carry i64) (local $v i64)
    (local.set $n (call $r_len (local.get $p))) (local.set $i (i32.const 0))
    (local.set $carry (i64.extend_i32_u (local.get $bit)))
    (block $done (loop $again
      (br_if $done (i32.ge_u (local.get $i) (local.get $n)))
      (local.set $v (i64.add (i64.shl (i64.extend_i32_u (call $r_limb (local.get $p) (local.get $i))) (i64.const 1)) (local.get $carry)))
      (call $r_set_limb (local.get $p) (local.get $i) (i32.wrap_i64 (i64.and (local.get $v) (global.get $MASK))))
      (local.set $carry (i64.shr_u (local.get $v) (i64.const 30)))
      (local.set $i (i32.add (local.get $i) (i32.const 1))) (br $again)))
    (if (i64.ne (local.get $carry) (i64.const 0))
      (then (call $r_set_limb (local.get $p) (local.get $n) (i32.wrap_i64 (local.get $carry))) (local.set $n (i32.add (local.get $n) (i32.const 1)))))
    (call $r_set_len (local.get $p) (local.get $n))
    (if (i32.ge_s (call $r_cmp (local.get $p) (global.get $R_M)) (i32.const 0))
      (then (call $r_sub (local.get $p) (local.get $p) (global.get $R_M)))))
  (func $wide_mag_mod_m (param $w i32) (param $dst i32)
    (local $i i32) (local $bit i32) (local $v i32)
    (call $r_zero (local.get $dst))
    (local.set $i (call $len (local.get $w)))
    (block $done (loop $limbs
      (br_if $done (i32.eqz (local.get $i)))
      (local.set $i (i32.sub (local.get $i) (i32.const 1)))
      (local.set $v (call $limb (local.get $w) (local.get $i)))
      (local.set $bit (i32.const 29))
      (block $bd (loop $bits
        (br_if $bd (i32.lt_s (local.get $bit) (i32.const 0)))
        (call $r_double_bit (local.get $dst) (i32.and (i32.shr_u (local.get $v) (local.get $bit)) (i32.const 1)))
        (local.set $bit (i32.sub (local.get $bit) (i32.const 1))) (br $bits)))
      (br $limbs))))
  (func $wide_euclid_mod_m (param $w i32) (param $dst i32)
    (call $wide_mag_mod_m (local.get $w) (local.get $dst))
    (if (i32.and (i32.lt_s (call $sign (local.get $w)) (i32.const 0)) (i32.ne (call $r_len (local.get $dst)) (i32.const 0)))
      (then (call $r_sub (local.get $dst) (global.get $R_M) (local.get $dst)))))
  (func $r_add_mod (param $dst i32) (param $a i32) (param $b i32)
    (local $i i32) (local $na i32) (local $nb i32) (local $n i32)
    (local $carry i64) (local $av i64) (local $bv i64) (local $s i64)
    (if (i32.eq (local.get $dst) (local.get $a)) (then (call $r_copy (global.get $R_TMP0) (local.get $a)) (local.set $a (global.get $R_TMP0))))
    (if (i32.eq (local.get $dst) (local.get $b)) (then (call $r_copy (global.get $R_TMP1) (local.get $b)) (local.set $b (global.get $R_TMP1))))
    (local.set $na (call $r_len (local.get $a))) (local.set $nb (call $r_len (local.get $b)))
    ...
    (block $done (loop $again
      (br_if $done (i32.ge_u (local.get $i) (local.get $n)))
      (local.set $av (if (result i64) (i32.lt_u (local.get $i) (local.get $na)) (then (i64.extend_i32_u (call $r_limb (local.get $a) (local.get $i)))) (else (i64.const 0))))
      (local.set $bv (if (result i64) (i32.lt_u (local.get $i) (local.get $nb)) (then (i64.extend_i32_u (call $r_limb (local.get $b) (local.get $i)))) (else (i64.const 0))))
      (local.set $s (i64.add (i64.add (local.get $av) (local.get $bv)) (local.get $carry)))
      (call $r_set_limb (local.get $dst) (local.get $i) (i32.wrap_i64 (i64.and (local.get $s) (global.get $MASK))))
      (local.set $carry (i64.shr_u (local.get $s) (i64.const 30)))
      (local.set $i (i32.add (local.get $i) (i32.const 1))) (br $again)))
    (if (i64.ne (local.get $carry) (i64.const 0))
      (then (call $r_set_limb (local.get $dst) (local.get $n) (i32.wrap_i64 (local.get $carry))) (local.set $n (i32.add (local.get $n) (i32.const 1)))))
    (call $r_set_len (local.get $dst) (local.get $n))
    (if (i32.ge_s (call $r_cmp (local.get $dst) (global.get $R_M)) (i32.const 0))
      (then (call $r_sub (local.get $dst) (local.get $dst) (global.get $R_M)))))
  (func $r_mul2_mod (param $p i32)
    (call $r_double_bit (local.get $p) (i32.const 0)))
  (func $r_add1_mod (param $p i32)
    (call $r_from_u32 (global.get $R_TMP1) (i32.const 1))
    (call $r_add_mod (local.get $p) (local.get $p) (global.get $R_TMP1)))
  (func $r_mod_small (param $p i32) (param $d i32) (result i32)
    (local $i i32) (local $r i64)
    (local.set $i (call $r_len (local.get $p))) (local.set $r (i64.const 0))
    (block $done (loop $again
      (br_if $done (i32.eqz (local.get $i)))
      (local.set $i (i32.sub (local.get $i) (i32.const 1)))
      (local.set $r (i64.rem_u (i64.add (i64.shl (local.get $r) (i64.const 30)) (i64.extend_i32_u (call $r_limb (local.get $p) (local.get $i)))) (i64.extend_i32_u (local.get $d))))
      (br $again)))
    (i32.wrap_i64 (local.get $r)))

  (func $day_count_mod (param $day i32) (param $foundation i32) (param $dst i32)
    (local $d i32) (local $cmpv i32)
    (local.set $cmpv (call $cmp (local.get $day) (local.get $foundation)))
    (if (i32.eqz (local.get $cmpv))
      (then (call $r_from_u32 (local.get $dst) (i32.const 1)) (return)))
    (if (i32.gt_s (local.get $cmpv) (i32.const 0))
      (then
        (local.set $d (call $sub (local.get $day) (local.get $foundation)))
        (call $wide_mag_mod_m (local.get $d) (local.get $dst))
        (call $r_mul2_mod (local.get $dst))
        (call $r_add1_mod (local.get $dst)))
      (else
        (local.set $d (call $sub (local.get $foundation) (local.get $day)))
        (call $wide_mag_mod_m (local.get $d) (local.get $dst))
        (call $r_mul2_mod (local.get $dst)))))

  (func $work_counts_mod (param $c i32) (param $t i32)
    (local $f i32) (local $d i32) (local $cmpct i32)
    (call $r_init_m)
    (local.set $f (call $from_i64 (i64.const -15055671)))
    (call $day_count_mod (local.get $c) (local.get $f) (global.get $R_ACTION))
    (call $day_count_mod (local.get $t) (local.get $f) (global.get $R_TARGET))
    (local.set $d (call $sub (local.get $t) (local.get $c)))
    (if (i32.lt_s (call $sign (local.get $d)) (i32.const 0)) (then (call $set_sign (local.get $d) (i32.const 1))))
    (call $wide_mag_mod_m (local.get $d) (global.get $R_DISTANCE))
    (call $r_add1_mod (global.get $R_DISTANCE))
    (call $r_add_mod (global.get $R_CONNECTION) (global.get $R_ACTION) (global.get $R_TARGET))
    (local.set $cmpct (call $cmp (local.get $t) (local.get $c)))
    (call $r_from_u32 (global.get $R_DIRECTION)
      (if (result i32) (i32.lt_s (local.get $cmpct) (i32.const 0))
        (then (i32.const 1))
        (else (if (result i32) (i32.eqz (local.get $cmpct)) (then (i32.const 2)) (else (i32.const 3)))))))

  (func (export "test_wide_counts_foundation") (result i32)
    (local $f i32)
    (local.set $f (call $from_i64 (i64.const -15055671)))
    (call $work_counts_mod (local.get $f) (local.get $f))
    (i32.and
      (i32.eq (call $r_mod_small (global.get $R_ACTION) (i32.const 1000)) (i32.const 1))
      (i32.and
        (i32.eq (call $r_mod_small (global.get $R_TARGET) (i32.const 1000)) (i32.const 1))
        (i32.and
          (i32.eq (call $r_mod_small (global.get $R_DISTANCE) (i32.const 1000)) (i32.const 1))
          (i32.and
            (i32.eq (call $r_mod_small (global.get $R_CONNECTION) (i32.const 1000)) (i32.const 2))
            (i32.eq (call $r_mod_small (global.get $R_DIRECTION) (i32.const 1000)) (i32.const 2)))))))

  (func (export "test_wide_counts_beyond_i64") (result i32)
    (local $c i32) (local $t i32)
    (local.set $c (call $from_i64 (i64.const -15055671)))
    ;; t = 2^90 + 17, construido sin pasar por i64.
    (local.set $t (call $new (i32.const 4)))
    (call $set_limb (local.get $t) (i32.const 0) (i32.const 17))
    (call $set_limb (local.get $t) (i32.const 3) (i32.const 1))
    (call $set_len (local.get $t) (i32.const 4))
    (call $set_sign (local.get $t) (i32.const 1))
    (call $work_counts_mod (local.get $c) (local.get $t))
    (i32.and
      (i32.eq (call $r_mod_small (global.get $R_DIRECTION) (i32.const 10)) (i32.const 3))
      (i32.and
        (i32.ne (call $r_len (global.get $R_TARGET)) (i32.const 0))
        (i32.ne (call $r_len (global.get $R_DISTANCE)) (i32.const 0)))))

  (func (export "fixture_wide_target_count_mod1000003") (result i32)
    (local $c i32) (local $t i32)
    (local.set $c (call $from_i64 (i64.const -15055671)))
    (local.set $t (call $new (i32.const 4)))
    (call $set_limb (local.get $t) (i32.const 0) (i32.const 17))
    ...
    (call $set_sign (local.get $t) (i32.const 1))
    (call $work_counts_mod (local.get $c) (local.get $t))
    (call $r_mod_small (global.get $R_TARGET) (i32.const 1000003)))
)
