...
(defun test-catalog ()
  (let ((cutlets (all-cutlet-entries))
    ...
    (check-equal "četru daļu no deviņām tulkojums" "četras daļas no deviņām" (cutlet-name-by-index 6) :test #'string=)
    (check-equal "trīs daļu no piecām tulkojums" "trīs daļas no piecām" (month-name-by-index 7) :test #'string=)
