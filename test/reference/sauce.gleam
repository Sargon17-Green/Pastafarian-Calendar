...
import reference/core.{
  type AnswerStream, type Grind, type SauceResult, type Stone, type WorkCounts,
}
...
fn hidden_grind_loop(grind: Int, x: Int, stone: Stone) -> Int {
  case grind > 7 {
    ...
    False -> {
      ...
      let next =
        core.save(
          core.square(x) + 3 * x + core.stone_value(stone, kind) + grind,
        )
  ...
  case i > 46 {
    ...
    False -> {
      ...
      build_visible_loop(
        ...
        stones,
        hidden,
        list.append(visible, [x]),
      )
    }
  }
}

fn visible_grind_loop(
  grinds: List(Grind),
  x: Int,
  p1: Int,
  p3: Int,
...
fn apply_drop_loop(
  ...
  case i > 46 {
    ...
    False -> {
      ...
      let next_bowls =
        stir_drop_positions(1, bowls, order, pours, stone, drop, i, [])
        |> list.reverse
      let latch = case i == 46 {
        True -> {
          order
...
fn build_pours(
  ...
  [
    core.save(
      core.square(drop) + stone.wheat * core.at1(bowls, first_id) + 3 * i,
    ),
    core.save(
      core.square(drop) + stone.barley * core.at1(bowls, second_id) + 5 * i,
    ),
    core.save(
      core.square(drop) + stone.salt * core.at1(bowls, third_id) + 7 * i,
    ),
    0,
    0,
    0,
...
fn stir_drop_positions(
  ...
  case position > 6 {
    ...
    False -> {
      ...
      let s =
        core.at1(old, bowl_id)
        + 2
        * core.at1(old, prev_id)
        + 3
        * core.at1(old, next_id)
      ...
      stir_drop_positions(position + 1, old, order, pours, stone, drop, i, [
        value,
        ..acc_by_id
      ])
    }
...
fn post_stir_loop(stir: Int, bowls: List(Int)) -> List(Int) {
  case stir > 12 {
    ...
    False -> {
      ...
      let next =
        post_stir_bowls_by_id(1, bowls, order, saved_bowl_sum, stir, [])
        |> list.reverse
...
fn post_stir_bowls_by_id(
  ...
  case bowl_id > 6 {
    ...
    False -> {
      ...
      let value =
        core.save(
          core.square(s) + 7 * core.at1(old, prev_id) * core.at1(old, next_id),
        )
      post_stir_bowls_by_id(bowl_id + 1, old, order, saved_bowl_sum, stir, [
        value,
        ..acc
      ])
    }
...
pub fn sauce(calculation_day: Int, target_day: Int) -> SauceResult {
  ...
  let #(after_drops, order_at_drop_46) =
...
pub fn next_bowl_in_drop_46_order(
  result: SauceResult,
  queried_bowl_id: Int,
) -> Int {
...
fn smallest_power_loop(base: Int, n: Int, k: Int, space: Int) -> #(Int, Int) {
  case space >= n {
    ...
    False -> {
      smallest_power_loop(base, n, k + 1, space * base)
    }
  }
}
