...
pub const m = 170_141_183_460_469_231_731_687_303_715_884_105_727

pub const tablets_day = -278_522

pub const foundation_day = -15_055_671
  ...
  case items {
    ...
    [head, ..tail] ->
      case head == wanted {
        ...
        False -> {
          position_of_loop(tail, wanted, position + 1)
        }
      }
  }
}
...
pub fn range_inclusive(first: Int, last: Int) -> List(Int) {
  case first > last {
    ...
    False -> {
      [first, ..range_inclusive(first + 1, last)]
    }
  }
}
...
fn falling_factorial_loop(n: Int, k: Int, j: Int, acc: Int) -> Int {
  case j >= k {
    ...
    False -> {
      falling_factorial_loop(n, k, j + 1, acc * { n - j })
    }
  }
}
...
pub fn work_counts(calculation_day: Int, target_day: Int) -> WorkCounts {
  ...
  let direction = case target_day < calculation_day {
    ...
    False ->
      case target_day == calculation_day {
        True -> {
          2
        }
        False -> {
...
pub fn permutation_unrank1(
  rank1: Int,
  items_ascending: List(Int),
) -> List(Int) {
...
fn permutation_loop(
  ...
  case slots_left == 0 {
    ...
    False -> {
      ...
      permutation_loop(next_rank0, next_remaining, slots_left - 1, [
        chosen,
        ..acc
      ])
    }
