...
pub fn gate_index_at_or_before(state: GateState, day: Int) -> #(Int, GateState) {
  let covered = ensure_gates_cover(state, day, day)
  #(binary_gate_before(covered, day, covered.min_index, covered.max_index), covered)
}
...
fn valid_year_pair(state: GateState, open_index: Int, close_index: Int) -> Bool {
  ...
  gaps >= core.min_gate_gaps_per_year && length >= core.year_min_days && length <= core.year_max_days
}

pub fn year_5000(calculation_day: Int, state: GateState) -> #(Year, GateState) {
  let covered = ensure_gates_cover(
    state,
...
fn sort_anchor_candidates(items: List(YearCandidate), state: GateState, sorted: List(YearCandidate)) -> List(YearCandidate) {
  case items {
    [] -> sorted
    [head, ..tail] -> sort_anchor_candidates(tail, state, insert_anchor(head, sorted, state))
  }
}

fn insert_anchor(candidate: YearCandidate, sorted: List(YearCandidate), state: GateState) -> List(YearCandidate) {
  case sorted {
    [] -> [candidate]
    [head, ..tail] -> {
...
pub fn next_year(calculation_day: Int, known: Year, state: GateState) -> #(Year, GateState) {
  let open_index = known.close_gate_index
  let open_day = gate_value(state, open_index)
  let covered = ensure_gates_cover(state, open_day, open_day + core.year_max_days + core.gate_gap_max)
  let candidates = collect_next_candidates(covered, open_index, open_index + 1, []) |> list.reverse
...
pub fn previous_year(calculation_day: Int, known: Year, state: GateState) -> #(Year, GateState) {
  let close_index = known.open_gate_index
  let close_day = gate_value(state, close_index)
  let covered = ensure_gates_cover(state, close_day - core.year_max_days - core.gate_gap_max, close_day)
  let candidates = collect_previous_candidates(covered, close_index, close_index - 1, []) |> list.reverse
  let sorted = sort_length_stable(candidates, [])
...
fn sort_length_stable(items: List(YearCandidate), sorted: List(YearCandidate)) -> List(YearCandidate) {
  case items {
    [] -> sorted
    [head, ..tail] -> sort_length_stable(tail, insert_length_stable(head, sorted))
  }
}

fn insert_length_stable(candidate: YearCandidate, sorted: List(YearCandidate)) -> List(YearCandidate) {
  case sorted {
    [] -> [candidate]
    [head, ..tail] ->
...
pub fn find_target_year(calculation_day: Int, target_day: Int, state: GateState) -> #(Year, GateState) {
...
pub fn unrank_distinct_indices(master_count: Int, k: Int, rank1: Int) -> List(Int) {
  unrank_distinct_loop(core.range_inclusive(1, master_count), k, rank1, 1, []) |> list.reverse
}
...
fn choose_cutlet_count(structure_sauce: core.SauceResult, year: Year) -> Int {
  ...
  let candidates = core.range_inclusive(core.min_cutlets, core.max_cutlets)
...
fn unrank_cutlet_partition(gaps: Int, slots: Int, required: Option(Int), rank1: Int) -> List(Int) {
...
fn choose_cutlet_partition(calculation_day: Int, structure_sauce: core.SauceResult, year: Year, cutlet_count: Int, state: GateState) -> #(List(Int), GateState) {
  ...
  let #(count, _) = count_cutlet_partitions(gaps, cutlet_count, 0, False, required, dict.new())
...
fn choose_cutlet_names(structure_sauce: core.SauceResult, cutlet_count: Int) -> List(Int) {
...
fn materialize_cutlets(year: Year, partition: List(Int), names: List(Int), state: GateState) -> List(Cutlet) {
  materialize_cutlet_loop(partition, names, year.open_gate_index, state, []) |> list.reverse
}

fn materialize_cutlet_loop(partition: List(Int), names: List(Int), cursor_gate: Int, state: GateState, acc: List(Cutlet)) -> List(Cutlet) {
  case partition, names {
    ...
    [part, ..parts], [name, ..rest_names] -> {
      ...
      materialize_cutlet_loop(parts, rest_names, close_gate, state, [cutlet, ..acc])
    }
...
fn choose_month_count(structure_sauce: core.SauceResult, year: Year) -> Int {
  ...
  let maximum = core.min_int(core.max_months, core.floor_div(length, core.min_month_days))
...
fn unrank_bounded(total: Int, slots: Int, low: Int, high: Int, rank1: Int) -> List(Int) {
...
fn choose_month_lengths(structure_sauce: core.SauceResult, year: Year, month_count: Int) -> List(Int) {
  let length = year.close_gate_day - year.open_gate_day
  let #(count, _) = count_bounded(length, month_count, core.min_month_days, core.max_month_days, dict.new())
  let stream = sauce.ask_bowl(structure_sauce, 3, core.seal_month_lengths)
  let rank = sauce.choose_rank(stream, count)
  unrank_bounded(length, month_count, core.min_month_days, core.max_month_days, rank)
}

type WeaveMemoKey = #(List(Int), Int, Int)
...
fn choose_month_weaving(structure_sauce: core.SauceResult, month_lengths: List(Int)) -> List(Int) {
  let #(count, _) = count_weavings(month_lengths, 0, 0, month_lengths, dict.new())
  let stream = sauce.ask_bowl(structure_sauce, 4, core.seal_month_weaving)
  let rank = sauce.choose_rank(stream, count)
  unrank_weaving(month_lengths, rank)
}

fn choose_month_names(structure_sauce: core.SauceResult, month_count: Int) -> List(Int) {
...
pub fn build_year_structure(calculation_day: Int, year: Year, state: GateState) -> #(YearStructure, GateState) {
  ...
  let #(partition, covered) = choose_cutlet_partition(calculation_day, structure_sauce, year, cutlet_count, state)
...
fn occurrence_count(items: List(Int), wanted: Int, through_position: Int) -> Int {
  occurrence_loop(items, wanted, through_position, 1, 0)
}

fn occurrence_loop(items: List(Int), wanted: Int, through_position: Int, position: Int, acc: Int) -> Int {
  case items {
    [] -> acc
    [head, ..tail] ->
...
pub fn calendar_date(calculation_day: Int, target_day: Int) -> CalendarDate {
  ...
  let #(year, state) = find_target_year(calculation_day, target_day, initial_state)
