# Catàleg de llengua font

Versió congelada: `1.0.0-stage01-frozen`.

## Regla general

Els noms amb significat lèxic es tradueixen pel significat, no per la forma gràfica de l'hebreu de la referència. Els topònims amb forma catalana establerta usen aquesta forma. Els noms inventats sense significat es transcriuen de manera determinista.

Per als noms inventats, la transcripció conserva l'ordre de fonemes i aplica aquestes decisions: /p b t d k g m n r l f v s z h/ conserven una lletra catalana directa; /ʃ/ s'escriu `x` davant d'una vocal i `ix` al final de mot després d'una vocal; les vocals /a e i o u/ es mantenen. Amb aquesta regla, els dos noms inventats del catàleg queden fixats com `Palguraix` i `Karxumav`.

Els topònims d'aquest catàleg queden fixats com `Lagaix`, `Accad`, `Èridu`, `Uruk`, `Nínive`, `Susa` i `Babilònia`. Aquesta decisió de presentació no participa en la semàntica.

## Mandonguilles

| canonicalIndex | text català |
|---:|---|
| 1 | bronze |
| 2 | guineu |
| 3 | ronyó |
| 4 | Lagaix |
| 5 | pensament |
| 6 | quatre parts de nou |
| 7 | Palguraix |
| 8 | papir |
| 9 | ramell |
| 10 | escorpí |
| 11 | cendra |
| 12 | blat |
| 13 | riu |
| 14 | riure |
| 15 | Accad |
| 16 | banya |
| 17 | la gerra buida |

## Mesos

| canonicalIndex | text català |
|---:|---|
| 1 | argila |
| 2 | magrana |
| 3 | colze |
| 4 | enveja |
| 5 | Èridu |
| 6 | pasta de dents |
| 7 | tres parts de cinc |
| 8 | Karxumav |
| 9 | lleopard |
| 10 | estany |
| 11 | boira |
| 12 | olíban |
| 13 | fus |
| 14 | costella |
| 15 | garrofa |
| 16 | Uruk |
| 17 | vergonya |
| 18 | camell |
| 19 | coure |
| 20 | pou |
| 21 | rovell |
| 22 | estrella |
| 23 | mel |
| 24 | melsa |
| 25 | pedra calcària |
| 26 | alegria |
| 27 | figa |
| 28 | Nínive |
| 29 | granota |
| 30 | pega |
| 31 | llàntia |
| 32 | la porta tancada |
| 33 | sèsam |
| 34 | clatell |
| 35 | plata |
| 36 | Susa |
| 37 | tempesta |
| 38 | ase |
| 39 | farina |
| 40 | recança |
| 41 | Babilònia |
| 42 | llengua |
| 43 | lli |
| 44 | sal |
| 45 | pera |
| 46 | arc |
| 47 | sorra |

El `canonicalIndex` és l'única identitat normativa. El text d'aquesta taula no es pot usar per ordenar, seleccionar, desclassificar ni construir claus semàntiques.
